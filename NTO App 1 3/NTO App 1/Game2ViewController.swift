...
extension SKNode {
    ...
    class func unarchive2FromFile(file: String) -> SKNode? {
        ...
        if let path = NSBundle.mainBundle().pathForResource(file, ofType: "sks") {
            ...
            let scene = archiver.decodeObjectForKey(NSKeyedArchiveRootObjectKey) as! Game2Scene
...
class Game2ViewController: UIViewController, MFMessageComposeViewControllerDelegate {
    

    
    var gameScene: Game2Scene!
    ...
    override func viewDidLoad() {
        ...
        if let scene = Game2Scene.unarchive2FromFile("GameScene") as? Game2Scene {
            ...
            let screenshotButton = UIBarButtonItem(title: "Submit", style: .Plain, target: self, action: Selector("screenshotButtonPressed"))
            let leadButton = UIBarButtonItem(title: "Leaderboard", style: .Plain, target: self, action: Selector("leadButtonPressed"))
    ...
    func leadButtonPressed() {
        ...
        leaderView.site = "https://docs.google.com/spreadsheets/d/1IvoLOb7GxsPegScA4aHIzIAm6SdMmtxdbAkpE-4YXQE/edit?usp=sharing"
    ...
    func screenshotButtonPressed() {
        ...
        let image = takeScreenshot(self.view!)
        
        UIImageWriteToSavedPhotosAlbum(image, nil, nil, nil)
        
        let infoAlert = UIAlertView()
    ...
    func showPlayerOptions() {
        ...
        let refreshAlert = UIAlertController(title: "Select Player", message: "Choose one. Game will stop when you lose and you can submit to leaderboard if you wish. Click back and return to game to change characters or tap to play again.", preferredStyle: UIAlertControllerStyle.Alert)
        
        refreshAlert.addAction(UIAlertAction(title: "Brian", style: .Default, handler: { (action: UIAlertAction) in
            self.gameScene.bird.texture = SKTexture(imageNamed: "brian")
